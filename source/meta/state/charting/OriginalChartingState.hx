...
class OriginalChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		// notetypes pq eu as vezes esqueço lol
		bpmTxt.text += '\n\n\nHold 1 to place goo notes.';
		bpmTxt.text += '\nHold 2 to place shoot notes.';
		bpmTxt.text += '\nHold 3 to place alt notes.';
		bpmTxt.text += '\nHold 4 to place dodge notes.';

		var playedSound:Array<Bool> = [false, false, false, false]; //Prevents earrape GF ahegao sounds
		curRenderedNotes.forEachAlive(function(note:Note) {
			if(note.strumTime <= Conductor.songPosition) {
				if(note.strumTime > lastConductorPos && ((playSoundBf.checked && note.mustPress)
					|| (playSoundDad.checked && !note.mustPress)) && FlxG.sound.music.playing && note.noteData > -1) {
